...
static const char broken[] = "broken";  // broken 默认的窗口名称
static char stext[1024];
static int screen;
static int sw, sh;           /* X display screen geometry width, height 显示器的宽度与高度*/
...
// 设置client的rules,实际上就是对比配置文件里面的rules
void
applyrules(Client *c)
{
    ...
    c->isscratchpad = 0;
    c->tags = 0;
    XGetClassHint(dpy, c->win, &ch);  // 获取win的信息，放到ch 中
...
restoreSession(void)
{
	...
	char *str = (char*)malloc(30 * sizeof(char)); // allocate enough space for excepted input from text file
	while (fscanf(fr, "%[^\n] ", str) != EOF) { // read file till the end
		long unsigned int winId;
		unsigned int tagsForWin;
        int num;
        Monitor *m;
		int check = sscanf(str, "%lu %u %d", &winId, &tagsForWin, &num); // get data
    ...
    // logtofile("restoreSession");

    for (Monitor *m = mons; m; m = m->next){
        for (Client *c = m->clients; c ; c = c->next) { // refocus on windows
            focus(c);
            restack(c->mon);
        }
    }
